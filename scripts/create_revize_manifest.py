...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create a manifest slice for municipalities whose platform is Revize."
    )
    parser.add_argument(
        "--seed-csv",
    ...
    parser.add_argument(
        "--out",
        default=str(ROOT / "data" / "manifests" / "revize_manifest.csv"),
        help="Output manifest CSV path.",
    )
    parser.add_argument(
        "--batch-id",
        default="batch_revize_refinement",
            ...
            rows.append(
                {
                    ...
                    "batch_id": str(batch_id).strip() or "batch_revize_refinement",
                    "priority": str(priority).strip() or "1",
                    "status": str(status).strip() or "pending",
                    "notes": "vendor_target=Revize",
                }
            )
...
def main() -> None:
    ...
    print(f"Created Revize manifest rows: {len(rows)}")
